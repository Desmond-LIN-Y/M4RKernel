...
cat( "Configuring IO ...\n")
cat("Model = covid_wave1_hsgpmatern32rd")

model_name <- "COVIMOD_wave1_m32_rd"
config <- yaml::read_yaml(paste0(hpc,"config/covid_wave1_hsgp.yml"))
model_path <- paste0(hpc,"stan_models/hsgp-m32-rd.stan")
export_path <- paste0(hpc, "output/stan_fits/covimod-hsgp-m32-rd.rds")
