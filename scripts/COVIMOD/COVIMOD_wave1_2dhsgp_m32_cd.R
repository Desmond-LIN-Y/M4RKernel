...
cat( "Configuring IO ...\n" )
model_name <- "covimod_wave1_2dhsgp-m32-cd"

cat(model_name)

config <- yaml::read_yaml(paste0(hpc,"config/covid_wave1_2dhsgp_cd.yml"))
model_path <- paste0(hpc,"stan_models/2dhsgp-m32_loop.stan")
export_path <- paste0(hpc, "output/stan_fits/2dhsgp-m32-cd-loop.rds")
...
stan_data$age_idx_std = NULL
stan_data$diff_idx_std = NULL
stan_data$strata_idx_std = NULL
stan_data$Xhsgp <- scale(make_nn_lowertri_idxset(85, 'cd'))
stan_data$S = make_S_matrix(30,30) # fix M
stan_data$sym_from_lowertri_idxset <- make_sym_from_lowertri_idx(85)
stan_data$A2 = 3655
indices <- matrix(NA, 85*85, 2)
 mm=0;
 for (r in 1:85){
       for (s in 1:85){
             mm = mm+1
...
stan_data$Xhsgp2 <- scale(indices)
