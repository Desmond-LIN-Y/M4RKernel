...
transformed data
{
  ...
  matrix[A*A, M1*M2] PHI2 = PHI_2D(L1,L2, S, Xhsgp2);
...
transformed parameters
{
  ...
  { // local scope
    ...
    f_MM = to_matrix(hsgp_m32_2d(z[,1], gp_alpha[MM], gp_rho_1[MM], gp_rho_2[MM], sqrt_LAMBDA, PHI)[sym_from_lowertri_idxset], A, A);
    f_MF = to_matrix(hsgp_m32_2d(z[,2], gp_alpha[MF], gp_rho_1[MF], gp_rho_2[MF], sqrt_LAMBDA, PHI2), A, A);
    f_FF = to_matrix(hsgp_m32_2d(z[,3], gp_alpha[FF], gp_rho_1[FF], gp_rho_2[FF], sqrt_LAMBDA, PHI)[sym_from_lowertri_idxset], A, A);
