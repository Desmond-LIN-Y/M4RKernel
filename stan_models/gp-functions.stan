...
{
  return 2*alpha / sqrt(rho) * inv((1/rho)^2 + ((pi()/2/L) * linspaced_vector(M, 1, M))^2) ^ 0.5;
}
...
matrix gp2d_matern12(array[] real x1, array[] real x2,
                     real alpha, real rho1, real rho2, matrix z)
{
  int A = size(x1);
  int B = size(x2);
  ...
  // Compute the exponentiated quadratic covariance function:
  matrix[A,A] K1 = gp_exponential_cov(x1, alpha, rho1) + diag_matrix(rep_vector(1e-9, A));
  matrix[B,B] K2 = gp_exponential_cov(x2, alpha, rho2) + diag_matrix(rep_vector(1e-9, B));
...
matrix gp2d_matern12_restruct(array[] real x1, array[] real x2,
                              real alpha, real rho1, real rho2, matrix z,
                              array[] int nn_idx)
{
  int A = size(x1);
  int B = size(x2);
  matrix[A,B] f_restruct = gp2d_matern12(x1, x2, alpha, rho1, rho2, z);
...
matrix hsgp_matern12(int A, real alpha, real rho1, real rho2, real L1, real L2, int M1, int M2,
            matrix PHI1, matrix PHI2, matrix z)
{
  vector[M1] sqrt_spd_1 = diagSPD_Matern12(alpha, rho1, L1, M1);
  vector[M2] sqrt_spd_2 = diagSPD_Matern12(alpha, rho2, L2, M2);
...
matrix hsgp_matern12_restruct(int A, real alpha, real rho1, real rho2, real L1, real L2, int M1, int M2,
                     matrix PHI1, matrix PHI2, matrix z, array[] int nn_idx)
{
  vector[M1] sqrt_spd_1 = diagSPD_Matern12(alpha, rho1, L1, M1);
  vector[M2] sqrt_spd_2 = diagSPD_Matern12(alpha, rho2, L2, M2);
