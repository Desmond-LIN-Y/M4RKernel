...
parameters
{
  ...
  vector<lower=0, upper=pi()/2>[G-1] gp_alpha_unif2;
...
transformed parameters
{
  ...
  vector<lower=0>[G-1] gp_alpha2 = tan(gp_alpha_unif2);  // Reparametrize Half-Cauchy for optimization in HMC
  array[G] matrix[A,A] log_cnt_rate;

  { // local scope
    matrix[A, A] f_MM, f_FF, f_MF;
    vector[A2] fmm = hsgp_m12m32_2d(z[,1], gp_alpha[MM],gp_alpha2[MM], gp_rho_1[MM], gp_rho_2[MM], L1, L2, S2, PHI) .* hsgp_m12_2d(z[,1], 1,  gp_rho_1[MM], gp_rho_2[MM], L1, L2, S2, PHI); 
    vector[A*A] fmf = hsgp_m12m32_2d(z[,2], gp_alpha[MF], gp_alpha2[MF], gp_rho_1[MF], gp_rho_2[MF], L1, L2, S2, PHI2) .* hsgp_m12_2d(z[,2], 1, gp_rho_1[MF], gp_rho_2[MF], L1, L2, S2, PHI2);
    vector[A2] fff = hsgp_m12m32_2d(z[,3], gp_alpha[FF], gp_alpha2[FF], gp_rho_1[FF], gp_rho_2[FF], L1, L2, S2, PHI) .* hsgp_m12_2d(z[,3], 1, gp_rho_1[FF], gp_rho_2[FF], L1, L2, S2, PHI);
...
model
{
  ...
  target += cauchy_lpdf(gp_alpha2 | 0, 1);
