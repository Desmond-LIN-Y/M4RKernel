...
data
{
 ...
 vector[2*A2-1] diff_std; // standardised age of contacted individuals
...
transformed data
{
  ...
  L_age1 = c_age1*max(diff_std);

  // Precompute HSGP basis functions
  matrix[2*A2-1, M_age1] PHI1 = PHI(A2, M_age1, L_age1, diff_std);
...
transformed parameters
{
  ...
  //age-specific effect to account for the under-reporting for females
  {
   ...
   matrix[A1, A2] raw_log_effect_ur_f; // GP approx for log_effect_age_under_report_f
    ...
    // local scope
    raw_f =  hsgp_matern32_restruct(A1, gpscale, lscale_1, lscale_2, L_age1, L_age2, M_age1, M_age2, PHI1, PHI2, z, NN_IDX);
    raw_log_effect_ur_f =  hsgp_matern52_restruct(A2, gpscale_ur, lscale_ur_1, lscale_ur_2, L_age1, L_age2, M_age1, M_age2, PHI1, PHI2, z_ur, NN_IDX);
