// This stan model considers both gender reported sexual contacts,
// input data: for both sexes
// community heter; age heter of female
// change the coding metod: starting from the female-male pattern
// model the under-reporting using 2D HSGP
// use Matern 5/2
// add back the baseline of the women especailly for the under-reporting effects
// including generated quantities block
// based on the stan code in the practical paper
// revisit the previous HSGP method in 2024
...
{
  vector[A_squared] f_fm; // GP approx
  vector[A_squared] log_effect_age_under_report_f; // GP approx for female under-reported effects
  ...
  // vector<lower=0>[Nmf_all] exp_log_under_reported_m_mf;
  vector[Nmf_all] log_under_reported_m_fm;

  //age-specific effect to account for the under-reporting for females
  {
    // local scope
    f_fm =  hsgp_m52_2d(z, gpscale, lscale_1, lscale_2, L_age1, L_age2, S2, PHI);
    log_effect_age_under_report_f =  hsgp_m52_2d(z_ur, gpscale_ur, lscale_ur_1, lscale_ur_2, L_age1, L_age2, S2, PHI);
  }

  {
    ...
    // exp_log_under_reported_m_mf = exp(log_m_mf);
    // women to men, including the under-reporting effects with the baseline
    log_under_reported_m_fm = log_m_fm + log_effect_under_report_baseline_f + log_effect_age_under_report_f[yfm_rowmajor_matrix_index_all];
    }
}
