...
parameters
{
  ...
  real<lower=0, upper=pi()/2> gp_alpha_unif;
  matrix[M1*M2, G-1] z; // HSGP basis function coefficients
}

transformed parameters
{
  real<lower=0> gp_alpha = tan(gp_alpha_unif); // Reparametrize Half-Cauchy for optimization in HMC
  array[G] matrix[A,A] log_cnt_rate;

  { // local scope
    matrix[A, A] f_MM, f_FF, f_MF;
    f_MM = to_matrix(hsgp_m32_2d(z[,1], gp_alpha, gp_rho_1[MM], gp_rho_2[MM], L1, L2, S2, PHI)[sym_from_lowertri_idxset], A, A);
    f_MF = to_matrix(hsgp_m32_2d(z[,2], gp_alpha, gp_rho_1[MF], gp_rho_2[MF], L1, L2, S2, PHI2), A, A);
    f_FF = to_matrix(hsgp_m32_2d(z[,3], gp_alpha, gp_rho_1[FF], gp_rho_2[FF], L1, L2, S2, PHI)[sym_from_lowertri_idxset], A, A);
